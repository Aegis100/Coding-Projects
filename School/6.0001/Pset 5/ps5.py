...
def scale_to_segment(scale, bpm=120):
    ...
    for pitch in scale:
        segment = add_piano_note(segment, pitch, len(segment), dur)
    return segment
...
def melody_to_segment(notes):
    ...
    length_tuple = sorted(notes[len(notes) - 1])
    length = length_tuple[1] + length_tuple[2]
    ...
    for tuple in notes:
        segment = add_piano_note(segment, tuple[0], tuple[1], tuple[2])
    return segment
...
def drumline_to_segment(drumline, bpm = 120):
    ...
    print (bpm)
    dur = bpm_to_beat_len(bpm)
    #TODO: Convert start_beat to the corresponding values in time.
    print (drumline, dur)
    drumline = drumline
    print (drumline)
    start_beat_list = []
    for beat in drumline:
        start_beat1 = ([beat[0], beat[1]*dur])
        start_beat_list.append(start_beat1)
    print (start_beat_list)
    start_beat_list = sorted(start_beat_list)
    #TODO: figure out the full length of the entire melody by finding the furthest *ending* time:
    name = drumline[len(drumline) - 1]
    name1 = name[0]
    length = start_beat_list[len(drumline) - 1] + len(get_drum_note(name1))
    ...
    for drum in range(len(drumline)):
        name = drumline[drum]
        print (name)
        name1 = name[0]
        print (name1)
        note = get_drum_note(name1)
        print (start_beat_list[drum])
        print (len(segment))
        segment = segment.overlay(note, start_beat_list[drum])
        #segment = add_note(segment, f'./samples/perc_{name1}.wav', start_beat_list[drum], dur=-100)
    return segment
...
if __name__ == "__main__":
    ...
    #-------------------------------
    #drumline = [('snare', 0), ('tom_floor', 0), ('kick', 500), ('snare', 1000), ('tom_floor', 1000)]
